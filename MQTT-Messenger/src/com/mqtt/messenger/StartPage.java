...
public class StartPage extends Activity {
	...
	static String broker = null, broker_incoming = null,
			broker_outgoing = null;
	...
	public void processLogin(View v) {
		...
		if (validateIPAddress(broker) == false) {
			Toast.makeText(getBaseContext(), "Invalid Server Address",
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		try {
			...
			Toast.makeText(getBaseContext(), "Invalid Port Addresses",
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (clientO == null) {
			try {
				clientO = (MqttClient) MqttClient.createMqttClient(
						broker_outgoing, null);
	...
	public void processRegister() {
		...
		pd = ProgressDialog.show(this, "Registering", "Please Wait..", true,
				false);
		new Thread() {
			public void run() {
				int t = 0;
	...
	private Handler handlerReg = new Handler() {
		...
		public void handleMessage(Message msg) {
			pd.dismiss();
			if (msg.what == 1) {
				SharedPreferences myPrefs = StartPage.this
				...
				Toast.makeText(getBaseContext(),
						"Registration Timedout, Try Again later",
						Toast.LENGTH_SHORT).show();
				clientO.terminate();
				client.terminate();
				client = null;
	...
	private Handler handlerLog = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				...
				Toast.makeText(getBaseContext(), "Login Correct",
						Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(getBaseContext(), "Login Incorrect. Try Again!",
						Toast.LENGTH_SHORT).show();
			} else if (msg.what == 3) {
				Toast.makeText(getBaseContext(), "Login Timedout. Try Again!",
						Toast.LENGTH_SHORT).show();
				clientO.terminate();
				client.terminate();
				client = null;
