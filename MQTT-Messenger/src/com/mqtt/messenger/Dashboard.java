...
public class Dashboard extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		phone_id = Settings.System.getString(getContentResolver(),
				Secure.ANDROID_ID);

		SharedPreferences myMessages = this.getSharedPreferences("myMessages",
				MODE_PRIVATE);
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (newTopic.equalsIgnoreCase(username) == true) {
				if (newData.startsWith("TOPICS")) {
					...
					listOfTopicsUpdated = true;
					listOfTopics = null;
					Toast.makeText(getBaseContext(), "No Topic Exists!",
							Toast.LENGTH_SHORT).show();
				} else if (newData.equalsIgnoreCase("TOPIC_CREATED")) {
					Toast.makeText(getBaseContext(),
							"Topic Created Successfully!", Toast.LENGTH_SHORT)
							.show();
				} else if (newData.equalsIgnoreCase("TOPIC_FAILED")) {
					Toast.makeText(getBaseContext(), "Topic Creation Failed!",
							Toast.LENGTH_SHORT).show();
				} else if (newData.equalsIgnoreCase("MESSAGE_FILTERED")) {
					Toast.makeText(getBaseContext(),
							"Your Message has been Filtered!",
							Toast.LENGTH_SHORT).show();
				}
			} else {
				...
				scroller.post(new Runnable() {
					public void run() {
						scroller.scrollTo(messageView.getMeasuredWidth(),
	...
	public void performMQTTAction(final int id) {
		...
		case 1:
			...
			if (listOfTopicsSubscribed.size() > 0) {
				...
				final View textEntryView = factory.inflate(
						R.layout.dialogpublish, null);
				AlertDialog.Builder alert = new AlertDialog.Builder(this);

				spin = (Spinner) textEntryView.findViewById(R.id.spinnerTopic);

				String[] topicsSubbed = new String[listOfTopicsSubscribed
						.size()];
				topicsSubbed = listOfTopicsSubscribed.toArray(topicsSubbed);

				aa = new ArrayAdapter<String>(this,
						android.R.layout.simple_spinner_item, topicsSubbed);
				spin.setAdapter(aa);
				...
				alert.setPositiveButton("Ok",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								Spinner spin = (Spinner) textEntryView
										.findViewById(R.id.spinnerTopic);
								EditText topicMessage = (EditText) textEntryView
										.findViewById(R.id.editMessage);
								String finalTopic;
								finalTopic = spin.getSelectedItem().toString();

								String msg2 = username + "#" + password + "#"
										+ topicMessage.getText().toString();
				...
				Toast.makeText(getBaseContext(),
						"Not Subscribed to Any Topic!", Toast.LENGTH_SHORT)
						.show();
			break;
		case 2:
			pd = ProgressDialog.show(this, "Fetching Topics", "Please Wait..",
					true, false);
		...
		case 3:
			...
			if (listOfTopicsSubscribed.isEmpty() == true) {
				...
				final View textEntryView2 = factory2.inflate(
						R.layout.dialogunsubscribe, null);
				...
				alert2.setPositiveButton("Ok",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								Spinner spin = (Spinner) textEntryView2
										.findViewById(R.id.spinnerTopic);
								mqttService.unsubscribeToTopic(spin
										.getSelectedItem().toString());
								removeTopicSubscribed(spin.getSelectedItem()
										.toString());
								Toast.makeText(getBaseContext(),
		...
		case 4:
			...
			final View textEntryView3 = factory3.inflate(
					R.layout.dialogcreatetopic, null);
			...
			alert3.setPositiveButton("Ok",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog,
							...
							EditText topic = (EditText) textEntryView3
									.findViewById(R.id.editCreateTopic);
	...
	public void publishToIncoming(String topic, String message) {
		...
		if (client == null) {
			try {
				client = (MqttClient) MqttClient.createMqttClient(
						broker_incoming, null);
		...
		new Thread() {
			public void run() {
				...
				if (listOfTopicsUpdated == true)
					handlerTopImmed.sendMessage(Message.obtain(handlerTopImmed,
							1));
				else
					handlerTopImmed.sendMessage(Message.obtain(handlerTopImmed,
							2));
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				if (listOfTopics != null) {
					...
					String[] finalTopicList = LOTList
							.toArray(new String[LOTList.size()]);

					spin = (Spinner) textEntryView1
							.findViewById(R.id.spinnerTopic);
					aa = new ArrayAdapter<String>(Dashboard.this,
					...
					alert1.setPositiveButton("Ok",
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
									...
									Spinner spin = (Spinner) textEntryView1
											.findViewById(R.id.spinnerTopic);
									mqttService.subscribeToTopic(spin
											.getSelectedItem().toString());
									addTopicSubscribed(spin.getSelectedItem()
											.toString());
									Toast.makeText(getBaseContext(),
				...
				Toast.makeText(getBaseContext(),
						"Fetching Topics Timedout, Try Again Later!",
						Toast.LENGTH_SHORT).show();
			}
		}
	};
	...
	public ArrayList<String> getTopicsSubscribed() {
		SharedPreferences myTopics = this.getSharedPreferences(
				"myTopicsSubbed", MODE_PRIVATE);
		String topicsString = myTopics.getString("topics", "");
		String[] topicsArray = topicsString.split("\\|");

		ArrayList<String> listOfTopicsSubscribed = new ArrayList<String>(
				Arrays.asList(topicsArray));
