...
public class MQTTService extends Service implements MqttSimpleCallback {
	...
	// This is how the Android client app will identify itself to the message
	// Broker
	...
	public void onCreate() {
		...
		// relating to background data use
	...
	synchronized void handleStart(Intent intent, int startId) {
		...
		if (isAlreadyConnected() == false) {
			...
			// Ongoing notification in the Status Bar showing that the Service
			// is Running
		 ...
		 * can reconnect immediately, without having to wait for the MQTT
		 * timeout
		 */
	...
	public void onDestroy() {
		...
		try {
			mqttClient.unsubscribe(listOfTopicsSubscribed
					.toArray(new String[listOfTopicsSubscribed.size()]));
	...
	private void notifyUser(String alert, String title, String body) {
		...
		NotificationManager nm = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
		Notification notification = new Notification(
				android.R.drawable.ic_dialog_email, alert,
	...
	public void publishToTopic(String topicName, String message) {
		...
		if (isAlreadyConnected() == false) {
			...
			try {
				mqttClient
						.publish(topicName, message.getBytes(), pubQoS, false);
				published = true;
	...
	public void rebroadcastStatus() {
		...
		// inform the application that the Service has successfully connected
	...
	public void publishArrived(String topic, byte[] payloadbytes, int qos,
			boolean retained) {
		// we protect against the phone switching off while we're doing this by
		// requesting a wake lock - we request the minimum possible wake lock -
		// just enough to keep the CPU running until we've finished
		...
		if (topic.equals(username) == false) {
			String msg = "\nTopic: " + topic + "\nMessage: " + messageBody
					+ "\n\n";
			SharedPreferences myMessages = this.getSharedPreferences(
					"myMessages", MODE_PRIVATE);
	...
	private boolean connectToBroker() {
		try {
			...
			// keep alive messages can be sent we schedule the first one of
			// these now
			scheduleNextPing();
